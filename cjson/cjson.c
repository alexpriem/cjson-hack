...
static PyObject* encode_object(PyObject *object, int flags);
static PyObject* encode_string(PyObject *object);
static PyObject* encode_unicode(PyObject *object);
static PyObject* encode_tuple(PyObject *object, int flags);
static PyObject* encode_list(PyObject *object, int flags);
static PyObject* encode_dict(PyObject *object, int flags);
static PyObject* encode_set(PyObject *object, int flags);
static PyObject* encode_datetime(PyObject *object);
static PyObject* encode_date(PyObject *object);
static PyObject* encode_time(PyObject *object);
static PyObject* encode_class(PyObject *object);
...
#define ENCODE_NORMAL 0
#define ENCODE_SLOPPY 1
...
decode_set(JSONData *jsondata)
{
    ...
    left = jsondata->end - jsondata->ptr;
    if ((left < 3) ||  (strncmp(jsondata->ptr, "set", 3)==0)) {
		PyErr_Format(JSON_DecodeError, "cannot parse JSON description: %.20s",
                     jsondata->ptr);
		return NULL;
	}
    ...
    c=*jsondata->ptr;
    if (c!='(') {
		PyErr_Format(JSON_DecodeError, "Expected ( at %.20s", jsondata->ptr);
		return NULL;
	}
   ...
   jsondata->ptr += 1;
    while (True) {
        ...
        c = *jsondata->ptr;
        if (c == 0) {
            PyErr_Format(JSON_DecodeError, "unterminated set starting at "
                         "position " SSIZE_T_F,
                         (Py_ssize_t)(start - jsondata->str));
            goto failure;;
        } else if (c == ']') {
            if (expect_item && items>0) {
                PyErr_Format(JSON_DecodeError, "expecting set item at "
                             "position " SSIZE_T_F,
...
/*
 * This function is an almost verbatim copy of set_repr() from
...
/*
 * This function is an almost verbatim copy of set_repr() from
 * Python's setobject.c with the following differences:
 *
 * - it uses encode_object() to get the object's JSON reprezentation.
 * - it uses [] as decorations isntead of () (to masquerade as a JSON array).
 */
...
static PyObject *
encode_set(PyObject *so, int flags)
{
    PyObject *result=NULL;
    PyObject *keys;
    int i,n,n2;
    PyObject *elements, *temp;
    PySetObject *s = (PySetObject *) so;


    keys=PySequence_List((PyObject *)so);
    n=PySequence_Length(keys);



	/* set wordt net zo gencodeerd als een list 
    temp=encode_list(set, flags);
    result=PyString_FromFormat("%s(",set->ob_type->tp_name);
    PyString_ConcatAndDel(&result, temp);
    temp=PyString_FromFormat(")");
    PyString_ConcatAndDel(&result, temp); */
...
static PyObject*
encode_tuple(PyObject *tuple, int flags)
{
    Py_ssize_t i, n;
    PyObject *s, *temp;
    ...
    for (i = 0; i < n; ++i) {
        s = encode_object(v->ob_item[i], flags);
        if (s == NULL)
            goto Done;
 ...
 */
static PyObject*
encode_list(PyObject *list, int flags)
{
    Py_ssize_t i;
    PyObject *s, *temp;
 ...
 */
static PyObject*
encode_dict(PyObject *dict, int flags)
{
    Py_ssize_t i;
    ...
    while (PyDict_Next((PyObject *)mp, &i, &key, &value)) {
        ...
        if (flags==ENCODE_NORMAL) {
            if (PyInt_Check(key) || PyLong_Check(key)) {
                key= PyObject_Str(key);   // XXX: leak key here?            
            }
        }
        ...
        if (!PyString_Check(key) && !PyUnicode_Check(key) &&
            ...
            PyErr_SetString(JSON_EncodeError, "JSON encodable dictionaries "
                            "must have string/unicode/integer/long keys");
            goto Done;
        }
        ...
        Py_INCREF(value);
        s = encode_object(key, flags);
        PyString_Concat(&s, colon);
        PyString_ConcatAndDel(&s, encode_object(value, flags));
        Py_DECREF(value);
        if (s == NULL)
            goto Done;
        status = PyList_Append(pieces, s);
...
//    PyObject *s, *temp;
    PyObject *result = NULL;
    PyObject *monthtxt, *daytxt, *hourtxt, *mintxt, *sectxt;
    ...
    result=PyString_FromFormat("\"%d", year);

    if (month<10)
        monthtxt=PyString_FromFormat ("-0%d", month);
    else
        monthtxt=PyString_FromFormat ("-%d",month);
    
    if (day<10)
        daytxt=PyString_FromFormat ("-0%d", day);
    else
        daytxt=PyString_FromFormat ("-%d",day);

    if (hour<10)
        hourtxt=PyString_FromFormat ("0%d", hour);
    else
        hourtxt=PyString_FromFormat ("%d",hour);

    if (minute<10)
        mintxt=PyString_FromFormat (":0%d", minute);
    else
        mintxt=PyString_FromFormat (":%d", minute);

    if (sec<10)
        sectxt=PyString_FromFormat (":0%d.%d\"", sec, usec);
    else
        sectxt=PyString_FromFormat (":%d.%d\"", sec, usec);

    PyString_ConcatAndDel (&result, monthtxt);
    PyString_ConcatAndDel (&result, daytxt);
    PyString_ConcatAndDel (&result, hourtxt);
    PyString_ConcatAndDel (&result, mintxt);
    PyString_ConcatAndDel (&result, sectxt);
    
    return result;
}

static PyObject*
encode_date(PyObject *date)

{
//    Py_ssize_t i, n;
//    PyObject *s, *temp;
    PyObject *result = NULL;
    PyObject *monthtxt, *daytxt;
    int year,month, day;
...
static PyObject*
encode_time(PyObject *time)

{
    PyObject *result = NULL;
    PyObject *mintxt, *sectxt;
    int  hour, minute, sec, usec;

    hour=PyDateTime_TIME_GET_HOUR (time);
    minute=PyDateTime_TIME_GET_MINUTE (time);
    sec=PyDateTime_TIME_GET_SECOND (time);
    usec=PyDateTime_TIME_GET_MICROSECOND (time);

    if (hour<10)
        result=PyString_FromFormat ("\"0%d", hour);
    else
        result=PyString_FromFormat ("\"%d",hour);
...
static PyObject*
encode_class(PyObject *class)

{
 PyObject *result = NULL, *classname=NULL, *backquote=NULL, *classtxt;
 
 classname=PyObject_GetAttr(class, PyString_FromString("__class__"));
 if (classname==NULL)  return result;
 
 classtxt=PyObject_Str(classname);
 backquote=PyString_FromString ("\"");
 if ((classtxt==NULL) || (backquote==NULL)) return result;
 
 result=PyString_FromString ("\"");
 if (result==NULL) return result;
 
 PyString_ConcatAndDel(&result, classtxt);
 PyString_ConcatAndDel(&result, backquote);
...
static PyObject*
encode_object(PyObject *object, int flags)
{
    if (object == Py_True) {
        ...
        return encode_list(object, flags);
    } else if (PyTuple_Check(object)) {
        return encode_tuple(object, flags);
    } else if (PyDict_Check(object)) { // use PyMapping_Check(object) instead? -Dan
        return encode_dict(object, flags);
        ...
        return encode_time(object);
    } else if (PySet_Check(object)) {
        return encode_set(object, flags);
    }
	 else	if ((PyClass_Check(object)) || (PyInstance_Check(object)) || (PyType_Check(object))) {
        return encode_class(object);
    } else {        
        PyObject *retval;
        retval=encode_class(object);
...
{
    return encode_object(object, ENCODE_NORMAL);
}

/* Encode object into its JSON representation, integer keys stay integers  */

static PyObject*
JSON_sloppyencode(PyObject *self, PyObject *object)
{
    return encode_object(object, ENCODE_SLOPPY);
}
...
static PyMethodDef cjson_methods[] = {
    ...
    {"sloppyencode", (PyCFunction)JSON_sloppyencode,  METH_O,
    PyDoc_STR("sloppyencode(object) -> generate the JSON representation for object\n"
                "integer keys are stored as integers instead of strings (*not* JSON compatible)"
              )},
